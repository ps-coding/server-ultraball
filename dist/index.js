...
wss.on("connection", (ws) => {
        ...
        try {
            ...
            switch (type) {
                case "create-game":
                    ...
                    const game = new Game(gameId, player, payload.cap, payload.lastPlayerKeepsPlaying, payload.isPublic);
                    wsGame = game;
                    games.push(game);
                    if (wsGame.gameEnded) {
                ...
                case "join-game":
                    ...
                    if (gameToJoin) {
                        if (gameToJoin.players.filter((p) => !p.bot).length >=
                            gameToJoin.cap ||
                            gameToJoin.cap <= 1 ||
                        ...
                        let playerId = Player.generateId();
                        while (gameToJoin.players.find((player) => player.id === playerId) ||
                            playerId === 0) {
                            playerId = Player.generateId();
                        }
                        const player = new Player(playerId, payload.name, ws);
                ...
                case "add-bot":
                    if (wsGame) {
                        ...
                        while (wsGame.players.find((player) => player.id === botId) ||
                            botId === 0) {
                ...
                case "load-move":
                    if (!(payload.playerId &&
                        ...
                        (payload.using === undefined ||
                            (Array.isArray(payload.using) &&
                                payload.using.every((item) => typeof item == "object" &&
                                    "amount" in item &&
                                    typeof item.amount == "number" &&
                                    item.edition &&
                                    typeof item.edition == "string" &&
                                    ["knife", "ball", "bazooka", "spiral"].includes(item.edition)))))) {
                        ws.send(JSON.stringify({
                            type: "error",
                            payload: { error: "Invalid Request" },
                        }));
                    ...
                    if (wsGame) {
                        wsGame.load(Object.assign(Object.assign({}, payload), { socket: ws }));
                        if (wsGame.gameEnded) {
                            games.splice(games.indexOf(wsGame), 1);
                        }
                    }
    ...
    ws.on("close", () => {
        var _a;
        if (wsGame) {
            const id = (_a = wsGame.players.find((p) => p.socket == ws)) === null || _a === void 0 ? void 0 : _a.id;
            if (id) {
